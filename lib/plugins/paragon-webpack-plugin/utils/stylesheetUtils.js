...
function insertStylesheetsIntoDocument({
  ...
  if (urls.default) {
    const existingDefaultLink = getDescendantByTag(headElement, 'link', [['href', `${urls.default}`]]);
  ...
  if (urls.brandOverride) {
    const existingBrandLink = getDescendantByTag(headElement, 'link', [['href', `${urls.brandOverride}`]]);
